...
export default function Calendario({ session }) {
  ...
  // ── REGISTRAR SUSCRIPCIÓN EN SUPABASE (OneSignal SDK v16) ───────────────
  // Guarda el player_id (onesignal_player_id) del dispositivo actual en la tabla
  // push_subscriptions, asociado a las materias y carrera del usuario.
  // Usa la nueva API de OneSignal v16 con OneSignalDeferred.
  useEffect(() => {
    if (!session?.token || materiasIds.length === 0) return;

    const registrarSuscripcion = async (oneSignalUserId) => {
    ...
    // SDK v16: usar OneSignalDeferred
    const deferred = window.OneSignalDeferred || [];
    deferred.push(async (OneSignal) => {
      // Esperar a que OneSignal esté listo
      try {
        // Obtener el ID de suscripción actual (puede ser null si no está suscripto)
        const subId = OneSignal.User?.PushSubscription?.id;
        if (subId) {
          await registrarSuscripcion(subId);
        }

        // Escuchar cambios de suscripción (cuando el usuario acepta/rechaza el permiso)
        OneSignal.User?.PushSubscription?.addEventListener("change", async (event) => {
          if (event.current?.id) {
            await registrarSuscripcion(event.current.id);
          }
        });
      } catch (err) {
        console.error("Error al registrar suscripción OneSignal:", err);
      }
    });
    // También asignamos de vuelta si OneSignalDeferred no existía aún
    if (!window.OneSignalDeferred) {
      window.OneSignalDeferred = deferred;
    }
  }, [materiasIds, session?.carreraId, session?.token]);
  ...
  const handleGuardarEvento = async (datosEvento) => {
    try {
      ...
      if (eventoEditando) {
        ...
        if (datosEvento.materiaId) {
          fetch(
            `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/enviar-notificacion`,
            {
              method: "POST",
              headers: {
              ...
              body: JSON.stringify({
                materiaId: datosEvento.materiaId,
                titulo: datosEvento.titulo,
                materiaNombre: datosEvento.materiaNombre,
              }),
            },
  ...
  const obtenerNombre = (userId) => {
    ...
    return payload?.unique_name || userId;
  };
